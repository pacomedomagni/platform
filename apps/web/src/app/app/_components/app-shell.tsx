...
export default function AppShellClient({
  ...
  // Initialize to the SSR default (false). Hydrate from localStorage in the
  // effect below so the server-rendered DOM matches the first client render
  // — reading localStorage in the lazy initializer ran on the server (false)
  // and on the client (true if sidebar_advanced=='true') and produced a
  // hydration mismatch (React error #418) whenever those diverged.
  const [showAdvanced, setShowAdvanced] = useState(false);
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const stored = localStorage.getItem('sidebar_advanced') === 'true';
    if (stored) setShowAdvanced(true);
  }, []);
  ...
  // The user is read from localStorage so it must be set after mount —
  // doing this in a useMemo runs at SSR (returns undefined) and at hydrate
  // (returns the parsed user object). The shell then renders nothing on
  // the server and the user's name on the client → React #418 hydration
  // mismatch. Defer the read into useEffect so the first client render
  // matches the SSR output, then fill in on the next render.
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        ...
        setUser({ name: parsed.name || parsed.email || 'User', email: parsed.email || '' });
      }
    } catch { /* ignore */ }
  }, []);
